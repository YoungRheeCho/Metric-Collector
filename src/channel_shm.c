...
typedef struct {
    ...
    size_t element_size; //channel이 다루는 데이터의 크기 (shared memory의 total size 계산에 이용)
    size_t max_elements; //데이터의 최대 개수 (shared memory의 total size 계산에 이용)
...
static int shm_channel_write(Channel* self, const void* data, size_t count) {
    ShmChannelState* st = (ShmChannelState*)self->impl_data;
    if(!st->writable || !st->mapped) return -1;
